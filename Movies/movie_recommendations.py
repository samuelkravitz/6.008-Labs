...
def compute_posterior(prior, likelihood, y):
    ...
    # first step.
    log_answer = np.asarray([])
    for val in range(len(prior)):
        x_log = np.log(prior[val])
        x_like = 0
        for obs in y:
            log_likely = np.log(likelihood[obs, val])
            x_like += log_likely
        x_like += x_log
        log_answer = np.append(log_answer, x_like)

    s = log_answer[0]
    for i in range(len(log_answer)-1):
        s = np.logaddexp(s, log_answer[i + 1])
    log_answer -= s
...
def infer_true_movie_ratings(num_observations=-1):
    ...
    for i in range(len(movie_id_list)):
        ratings = movie_data_helper.get_ratings(movie_id_list[i])
        if num_observations == -1:
            num_ratings = len(ratings)
        else:
            num_ratings = num_observations
        posteriors[i] = compute_posterior(prior, likelihood, ratings[:num_ratings])
...
def main():
    ...
    # print("Expected answer:")
    # print(0.811278124459)
    movie_list = movie_data_helper.get_movie_id_list()
    max_ratings = (infer_true_movie_ratings(-1)[1])
    for i in range(len(max_ratings)):
        if max_ratings[i] == 6:
    ...
    #     entropies.append(sum(compute_true_movie_rating_posterior_entropies(i)) / num_movies)
    #     print(i)
    # plt.plot(entropies)
    # plt.title("Computing Entropy as Observations Increase")
    # plt.show()
    # END OF ERROR CHECKS
    # -------------------------------------------------------------------------
